...
class Message extends Coolsms
{
    ...
    public function send($options)
    {
        ...
        if(isset($options->country)) $args->country = $options->country;
        if(isset($options->subject)) $args->subject = $options->subject;
        if (isset($options->imageId)) $args->imageId = $options->imageId;
        if (isset($options->scheduledDate)) $args->scheduledDate = $options->scheduledDate;
        if (isset($options->kakaoOptions)) $args->kakaoOptions = $options->kakaoOptions;
        $object = new \stdClass();
        $object->messages = array($args);
        $object->groupOptions = new \stdClass();
        if (isset($options->appId)) $object->groupOptions->appId = $options->appId;
        if (isset($options->appVersion)) $object->groupOptions->appVersion = $options->appVersion;
        if (isset($options->mode)) $object->groupOptions->mode = $options->mode;
        if (isset($options->forceSms)) $object->groupOptions->forceSms = $options->forceSms;
        if (isset($options->onlyAta)) $object->groupOptions->onlyAta = $options->onlyAta;
        if (isset($options->osPlatform)) $object->groupOptions->osPlatform = $options->osPlatform;
        if (isset($options->devLanguage)) $object->groupOptions->devLanguage = $options->devLanguage;
        if (isset($options->sdkVersion)) $object->groupOptions->sdkVersion = $options->sdkVersion;
